...
function bindEvents() {
  ...
  document.addEventListener('keydown', e => { if (['INPUT','TEXTAREA','SELECT'].includes(e.target?.tagName)) return; if (e.code==='Space') { e.preventDefault(); state.annotator.setPanMode?.(true); return } if ((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='s') { e.preventDefault(); saveReview(); return } if (e.key==='ArrowLeft') { e.preventDefault(); step(-1) } if (e.key==='ArrowRight') { e.preventDefault(); step(1) } if (e.key==='Escape') cancelTool() })
...
async function openImage(item){ if(!item)return; await flushPendingSave(); cancelTool(); const token=++state.openToken; state.current=item; state.currentMeta=item.meta||state.meta.get(item.name)||null; state.dirty=false; updateHeaderForCurrent(); renderList(); setSavedStatus('불러오는 중…',''); try{ const [imageUrl,labels,review]=await Promise.all([ensureImageUrl(item),loadLabelData(item.name),loadReviewData(item.name)]); if(token!==state.openToken)return; await state.annotator.loadImage(imageUrl); if(token!==state.openToken)return; state.originalPolygons=Array.isArray(labels.polygons)?labels.polygons:[]; state.originalLandmarks=Array.isArray(labels.landmarks)?labels.landmarks:[]; if(labels.start_label)state.annotator.startLabel=labels.start_label; state.review=normalizeReview(review,item); $('lrMemo').value=state.review.memo||''; renderCombined(); renderAdditionList(); renderOriginalLandmarks(); applyLabelVisibility(); setSavedStatus(state.review.done?'검토 완료':'저장됨',state.review.done?'done':''); updateHeaderForCurrent(); preloadAround(state.list.findIndex(x=>x.name===item.name)) }catch(e){ if(token!==state.openToken)return; showError(e); setSavedStatus('불러오기 실패','error') } }
...
function armTool(label,mode,display){ if(!state.current)return; cancelTool(false); const beforeIds=new Set((state.annotator.polygons||[]).map(p=>String(p.id))); state.activeTool={label,mode,display,beforeIds}; state.annotator.readOnly=false; state.annotator.setTool('draw'); state.annotator.setPendingLabel(label,mode); $('lrToolStatus').textContent=mode==='point'?`${display}: 위치를 1번 클릭하세요`:`${display}: 기준점을 2번 클릭하세요`; $('lrToolStatus').classList.add('active'); $('lrCancelTool').disabled=false }
...
function preloadAround(index){ if(!state.list.length||index<0)return; const token=++state.prefetchToken, items=[]; for(let n=1;n<=PREFETCH_AHEAD;n++)if(state.list[index+n])items.push(state.list[index+n]); if(state.list[index-1])items.push(state.list[index-1]); if(!items.length){$('lrPreload').textContent='';return} ;(async()=>{let done=0;$('lrPreload').textContent=`다음 이미지 미리 읽기 0/${items.length}`; for(const item of items){if(token!==state.prefetchToken)return; try{await Promise.all([ensureImageUrl(item),loadLabelData(item.name),loadReviewData(item.name)])}catch{} done++; if(token===state.prefetchToken)$('lrPreload').textContent=`다음 이미지 미리 읽기 ${done}/${items.length}` } if(token===state.prefetchToken)$('lrPreload').textContent='다음 이미지 준비됨'})() }
