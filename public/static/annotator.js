  ...
  onMouseDown(e) {
    ...
    if (this.tool === 'draw') {
      ...
      if (this.pendingLabel && this.pendingLabelMode === 'circle' && !this.drawing) {
        const cscale = this.stage.scaleX() || 1
        const ccolor = getRegionColor(this.pendingLabel)
        if (!this._circleFirst) {
          this._circleFirst = { x: pos.x, y: pos.y }
          this._circleFirstDot = new Konva.Circle({ x: pos.x, y: pos.y, radius: 4 / cscale, fill: ccolor, listening: false })
          this._circlePreview = new Konva.Circle({ x: pos.x, y: pos.y, radius: 0, stroke: ccolor, strokeWidth: 2 / cscale, dash: [6 / cscale, 4 / cscale], listening: false })
        ...
        this._commitCircle(pos.x, pos.y, cr)
        return
      }
